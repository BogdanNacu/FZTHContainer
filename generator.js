...
function generateTable(container, initialHotels) {
	var constants = {
        ...
        ROW_ADD_MODE_CLASS: 'row-hotel-edit',
    ...
    function onOperationCancel(sender) {
     ...
     function editTextControlCellToRow(row, controlRoll, val)
     {
         var col = $('<td/>');
         var input = $('<input/>', {
             'value': val,
             'type': 'text',
             'data-crole': controlRoll
         });
         ;
         col.append(input);
         row.append(col);
     ...
     function enterEditMode(initRow, hotel) {
         var row = $('<tr/>', {
             'class': constants.ROW_EDIT_MODE_CLASS
         });

         editTextControlCellToRow(row, 'name', hotel.name);
         editTextControlCellToRow(row, 'description', hotel.description);
         editTextControlCellToRow(row, 'country', hotel.country);
         editTextControlCellToRow(row, 'city', hotel.city);
         editTextControlCellToRow(row, 'addedDate', hotel.addedDate);
         editTextControlCellToRow(row, 'startPrice', hotel.startPrice);

         addOperationsCellToRowForAddUpdate(row);
         initRow.html(row.html());
